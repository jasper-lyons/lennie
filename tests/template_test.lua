...
describe('Template', function (it, describe)
  describe(':parse(template)', function (it)
    ...
    it('should allow calling methods on values in the context', function ()
      local result = Template:new('{{ value:len() }}'):render({ value = 'hello' })
      assert(result == '5')
    end)

    it('should throw any errors while interpreting the template', function ()
      assert(not pcall(function ()
        Template:new('{{ not valid lua }}'):render()
      end))
    end)
  end)
end)
